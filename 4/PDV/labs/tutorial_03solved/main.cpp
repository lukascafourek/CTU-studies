...
double integrated_function(double integrationRange) {
    const int num_steps = 1000 * static_cast<int>(std::ceil(integrationRange*integrationRange*integrationRange*integrationRange));
...
int main() {
...

    {
        ...
        for(unsigned i = 0 ; i < solvers.size() ; i++) {
            ...
            long elapsed = static_cast<long>(duration_cast<microseconds>(end - begin).count());
