...
generate_instance_with_query(const size_t length_of_query, const size_t count_of_rows, const bool bool_value_to_ensure,
    ...
    for (size_t i = 0; i < length_of_query; ++i) {
        ...
        test_row_t value_t = dist(rng) + (test_row_t)(count_of_rows / std::thread::hardware_concurrency());
        if (!is_predicate_true[i]) value_t = -1;
...
// zbyle parametry odpovidaji parametrum metody generate_instance_with_query v hlavickovem souboru
std::pair<std::vector<test_row_t>, std::vector<predicate_t<test_row_t>>>
generate_instance_with_query_conjunction_t(const size_t length_of_query, const size_t count_of_rows) {
    ...
    for (size_t i = 0; i < length_of_query; ++i) {
        ...
        std::uniform_int_distribution<test_row_t> dis_index(0, (test_row_t)((count_of_rows - 1) / 20));
        // zajima nas rozpeti hodnot - od - kam
        auto start_index_number = dis_index(rng);
        auto end_index_number = (test_row_t)(count_of_rows - dis_index(rng));
        // ale i jejich delitelnost nejakym cislem
        std::uniform_int_distribution<test_row_t> dis_space(2, 5);
...
generate_instance_with_query(Operation operation, bool is_query_evaluated_true, size_t length_of_query,
    ...
    rng.seed(SEED); // NOLINT(*-msc51-cpp)
