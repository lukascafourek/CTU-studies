...
namespace maze {
    ...
    public:
        maze_state(const state_ptr predecessor, unsigned int cost, std::vector<unsigned int> conf,
        ...
        std::vector<state_ptr> next_states() const override {
            ...
            if (!((*MAZE)[(conf[0] - 1) * SIZE + conf[1]])) {
                tmp_conf[0] = conf[0] - 1;
                succ.emplace_back(
                    std::make_shared<maze_state<SIZE, UNIFORM>>(this->shared_from_this(), current_cost() + cost,
                                                                tmp_conf, MAZE, GOAL));
                tmp_conf[0] = conf[0];
            }
    ...
    public:
        ...
        state_ptr get_root() {
            ...
            std::cout << "Domena Bludiste\n";
            std::cout << "Sirka plochy = " << WIDTH << ", vyska plochy = " << HEIGHT << ", uniformni ceny = "
                      << (UNIFORM ? "ano" : "ne") << ", seed = " << SEED << "\n";
            std::cout << "Plocha = \n";
            ...
            if ((*maze)[rootState[0] * WIDTH + rootState[1]] || (*maze)[(*goal)[0] * WIDTH + (*goal)[1]]) {
                std::cout << "Vsechny pokusy vygenerovat validni bludiste selhaly. Zkuste pouzit LICHE rozmery."
                          << "\n";
                exit(1);
            }
