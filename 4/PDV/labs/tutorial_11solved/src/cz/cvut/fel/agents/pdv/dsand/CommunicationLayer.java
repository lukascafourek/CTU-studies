...
public class CommunicationLayer {
  ...
  public void deliveryCycle() {
    ...
    while (count > 0) {
      ...
      ChannelInfo currChInfo = toDeliver.get(idx);

      if (!currChInfo.channel.isEmpty() && currChInfo.channel.peek().deliveryTime <= currentTick) {
        inboxes.get(currChInfo.dst).add(currChInfo.channel.poll().unpack());
      } else {
        toDeliver.set(idx, toDeliver.get(--count));
        if (!currChInfo.channel.isEmpty()) {
          currChInfo.update();
          leastMessages.add(currChInfo);
        }
      }
    }
  }
  ...
  public void send(String sender, String recipient, Message message) {
    if (rnd.nextDouble() > config.getReliability(sender, recipient)) {
      return;
    }
  ...
  private static class ChannelInfo implements Comparable<ChannelInfo> {
    ...
    public void update() {
      if (channel.isEmpty()) {
        ...
        leastMessage = channel.peek().deliveryTime;
      }
    }
  }
}
