...
solution_t eval(const std::string& name, std::vector<elem_t> & unsorted,
...
int main() {
    ...
    //see CMakeLists.txt
    //eval("std::sort_parallel(...)    ",unsorted,[](std::vector<elem_t> & to_sort) {
    //    std::sort(std::execution::par_unseq,to_sort.begin(), to_sort.end());
    //},reference.get());
    eval("my_mergesort_parallel (...)",unsorted,&my_mergesort_parallel,reference.get());
    ...
    eval("counting_parallel_atomic(...)   ", unsorted, &counting_parallel_atomic, reference.get());
    eval("counting_parallel_histogram(...)",unsorted,&counting_parallel_histogram,reference.get());
